...
class GetSchooled(tk.Tk):
	...
	def __init__(self, *args, **kwargs):
		...
		for F in (StartPage, PageOne, PageTwo, PageThree, PageFour, PageFive, PageSix, PageSeven, PageEight, PageNine, PageTen, PageEleven, PageTwelve, PageThirteen, PageFourteen, PageFifteen, PageSixteen, PageSeventeen, PageEighteen, PageNineteen, PageTwenty, ResultsPage, DisplayPage_Scrollbars):
...
class ResultsPage(tk.Frame):
	...
	def __init__(self, parent, controller):
		...
		displayButton = tk.Button(self, text="SHOW ME", 
			command=lambda: controller.show_frame(DisplayPage_Scrollbars))
...
class DisplayPage_Scrollbars(tk.Frame):
	'''this class instantiates as a whole-window frame'''
	def __init__(self, parent, controller):
		tk.Frame.__init__(self, parent)
		...
		myframe.pack()
		print("myframe loaded")

		canvas = tk.Canvas(myframe)
		frame = tk.Frame(canvas, background='#00ffff')
		myscrollbar = tk.Scrollbar(canvas,orient="vertical",command=canvas.yview)
		...
		frame.bind("<Configure>", (canvas.configure(scrollregion=canvas.bbox("all"),width=200,height=200)))
		print("scrollbar loaded")

		label = tk.Label(frame, text="School Information", font=LARGE_FONT)
		label.pack(pady=10, padx=10)
		print("static label loaded")

		calcButton = tk.Button(frame, text="Show Detailed Information!", command=lambda: controller.show_frame(DisplayPage))
		calcButton.pack()
		calcButton.bind("<1>", lambda event, self=self: self.get_school_info())
		print("calcbutton loaded")

		# pulls hypothetical colleges as a string returned from college_selection function
		results = tk.StringVar()
		results.set(' ') 
		# results.set('This is the display')
		display = tk.Label(frame, textvariable=results)
		display.pack()
		print("results texst loaded")

		startOverButton = tk.Button(frame, text="START OVER", 
			command=lambda: controller.show_frame(StartPage))
		startOverButton.pack(pady=25)
		startOverButton.bind("<1>", erase_selections)
		print("start over button loaded")
	...
	def get_school_info(self):
		...
		results.set(result)
		print("UI Method: ",results)
		display = tk.Label(frame, textvariable=results, font=SECOND_FONT)
		display.pack()
