...
df = pd.read_csv('consolidated_schools.csv')
df = df.iloc[:142]
choices_df = pd.read_csv('fictional_project_scenarios.csv')
...
def college_selection(row_number):
	...
	to the first column of the ProjectScenarios_revisedMP csv file".
	'''
...
def score_schools(choices_df,df,match_level):
	...
	match_level = the C level used in the log regression to determine strictness, as chosen by the user (user input: 0-4)
	'''
	match_level_list = [1000,1,0.1,0.01,0.05] # 0 = 1000, 1 = 1, 2 = 0.1, 3 = 0.01, 4 = 0.05. The larger the matching level (C parameter), the stricter the matching will be 
	...
	for i in range(len(df)):
		...
		#Create a list containing all information for one actual school, in the order of the choices_df columns
...
def display_matched_schools():
	'''
	This function displays information about the top 4 matched schools or, if there are less than 4, the names of the schools that matched
	'''
	
	# Only show the top 4 results because >4 does not fit and adding scroll capabilities in tkinter is difficult 
	matched_schools = calculate_matched_schools(list_of_answers)
	if len(calculate_matched_schools(list_of_answers)) > 4:
		matched_schools = calculate_matched_schools(list_of_answers)[:4]
...
def display_school_info():
	'''
	This fucntion displays information about the matched schools, like SAT score and admissions rate
	'''
